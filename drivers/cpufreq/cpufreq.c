...
static unsigned int Lscreen_off_scaling_mhz = 1728000;
static unsigned int Lscreen_off_scaling_mhz_orig = 1728000;
...
static ssize_t store_scaling_booted(struct cpufreq_policy *policy, const char *buf, size_t count)
{
	...
	if (value == 1)
	{
		...
		new_policy.user_policy.max = 1728000;
		ret = __cpufreq_set_policy(policy, &new_policy);
	}
	else
		isBooted = 0;
